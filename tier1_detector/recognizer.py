"""
Tier 1b — Speech Recognizer (faster-whisper)

Offline Indonesian speech-to-text using the faster-whisper tiny model.
Auto-downloads from HuggingFace on first run (~75MB, cached after that).
...
- faster-whisper tiny has excellent Indonesian support out of the box.
- It runs at 4x the speed of original Whisper due to CTranslate2 backend.
- The tiny model is fast enough for near-real-time short utterances.
"""
...
class WhisperRecognizer:
    ...
    def __init__(self, model_size: str = "tiny"):
        ...
        logger.info("(Model auto-downloads from HuggingFace on first run)")

        from faster_whisper import WhisperModel

        # Use CPU with int8 quantization — fastest on laptop/RPi
        self._model = WhisperModel(
            model_size,
            device="cpu",
            compute_type="int8",
        )
        self._language = "id"  # force Indonesian

        logger.info("✅ Whisper '%s' model ready (CPU/int8)", model_size)
