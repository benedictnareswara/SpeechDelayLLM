...
from tier1_detector.phoneme_extractor import extract_phoneme
...
async def process_text(request: TextInputRequest):
    ...
    """
    # Run through Tier 1 phoneme extraction (skip VAD/Vosk)
    phoneme_result = extract_phoneme(request.text, vosk_confidence=0.9)

    # Run through Tier 2+3 routing
...
async def websocket_stream(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            phoneme_result = extract_phoneme(data, vosk_confidence=0.8)
            response = await _semantic_router.route(phoneme_result)
